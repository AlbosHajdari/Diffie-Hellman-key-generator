...
public class DiffieHellmanAlgorithm extends JFrame {
	...
	public void gjenerimi()
	{
		try
		{
			...
			if((new BigInteger(txtN.getText()).compareTo(new BigInteger(txtG.getText())))>0)
			{
				state = false;
				lbln.setText("Numri N duhet të jetë më i vogël se numri prim G");
			}
			if(state)
			{
				try
				{
					...
					//Numri i pare prim
					BigInteger biN = new BigInteger(txtN.getText());
					//Numri i dyte prim
					BigInteger biG = new BigInteger(txtG.getText());
					...
					BigInteger numriIgjeneruarNgaA = biN.modPow(biA,biG);
					// numriIdytePrim(biG)^(numriSekretiB(biB)) modulo (NumriIparePrim(biN))
					BigInteger numriIgjeneruarNgaB = biN.modPow(biB, biG);
					
					//celesi sekret i gjeneruar nga A
					celesi1 = numriIgjeneruarNgaB.modPow(biA, biG);
					//celesi sekret i gjeneruar nga B
					celesi2 = numriIgjeneruarNgaA.modPow(biB, biG);
